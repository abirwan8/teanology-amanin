...
const TeaMenusAngry = () => {
  ...
  useEffect(() => {
    Axios.get(`http://localhost:5000/moodbevs/angry/${tokoId}`).then((response) => {
    ...
    <>
      {Object.keys(groupedBevList).map((type, index) => {
        ...
        if (filteredList.length > 0) {
          return (
            <Container fluid className="menu__box-angry" key={index}>
              <h3 className="type-menu-angry">{type}</h3>

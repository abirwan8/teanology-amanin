...
const TeaMenusHappy = () => {
  ...
  useEffect(() => {
    Axios.get(`http://localhost:5000/moodbevs/happy/${tokoId}`).then((response) => {
    ...
    <>
      {Object.keys(groupedBevList).map((type, index) => {
        ...
        if (filteredList.length > 0) {
          return (
            <Container fluid className="menu__box-happy" key={index}>
              <h3 className="type-menu-happy">{type}</h3>
